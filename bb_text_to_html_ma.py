...
#==============
# This function loads a random multiple-answer question from a file.
def load_single_question_from_file(file_path: str) -> tuple:
	"""
	Load a single random multiple-answer question from a file.
	...
	# Check if the question format is valid based on specific criteria (MA)
	if parts[0] != "MA" or len(parts) < 4 or (len(parts) - 2) % 2 != 0:
...
# Updated JavaScript function for MA questions (multiple answers)
def generate_javascript(hex_value) -> str:
    """
    Generate JavaScript code for validating multiple answers to a MA (multiple answer) question.
    ...
    # Define a function to check the selected answers, using the unique hex value
    ...
    # Find the correct options by checking the custom data attribute
    javascript_html += " const correctOptions = Array.from(options).filter(function(option) {\n"
    javascript_html += "   return option.dataset.correct === 'true';\n"
    javascript_html += " });\n"

    # Find the selected options by checking which checkboxes are checked
    javascript_html += " const selectedOptions = Array.from(options).filter(function(option) {\n"
    javascript_html += "   return option.checked;\n"
    javascript_html += " });\n"
    ...
    javascript_html += " if (selectedOptions.length === correctOptions.length) {\n"

    # Now check if all selected options are correct
    javascript_html += "   const allCorrect = selectedOptions.every(function(option) {\n"
    javascript_html += "     return correctOptions.some(function(correctOption) {\n"
    javascript_html += "       return correctOption === option;\n"
    javascript_html += "     });\n"
    javascript_html += "   });\n"

    # If all selected options are correct
    javascript_html += "   if (allCorrect) {\n"
    javascript_html += "     resultDiv.style.color = 'green';\n"
    javascript_html += "     resultDiv.textContent = 'CORRECT';\n"

    # If not all selected options are correct
    javascript_html += "   } else {\n"
    javascript_html += "     resultDiv.style.color = 'red';\n"
    javascript_html += "     resultDiv.textContent = 'Incorrect';\n"
    javascript_html += "   }\n"
    
    # If the number of selected options is not the same as the number of correct options
    javascript_html += " } else {\n"
    javascript_html += "   resultDiv.style.color = 'red';\n"
    javascript_html += "   resultDiv.textContent = 'Incorrect, wrong number of answers selected.';\n"
    javascript_html += " }\n"

    # If no options were selected
    javascript_html += " if (selectedOptions.length === 0) {\n"
