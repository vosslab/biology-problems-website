...
- Regenerate subject indexes, nav, and the self-test progress manifest:
  ```bash
  source source_me.sh && python3 bioproblems.py pages
  ```
- Run one BBQ task CSV:
  ```bash
  source source_me.sh && python3 bioproblems.py bbq --flat \
    --tasks task_files/other_tasks.csv
  ```
...
- Generated progress data:
  ...
  is produced by [`bioproblems.py pages`](../bioproblems.py) from reachable topic
  pages and their self-test includes.

## Page generation

`bioproblems.py pages` regenerates subject index pages, topic pages, the generated
nav block in [mkdocs.yml](../mkdocs.yml), and the self-test manifest from
[topics_metadata.yml](../topics_metadata.yml). Run it from the repo root:
...
`bioproblems.py bbq` runs one or all task CSVs. Operational usage lives in
[BBQ_TASKS_USAGE.md](BBQ_TASKS_USAGE.md). For the CSV schema and how the `topic`
column resolves through aliases, see
