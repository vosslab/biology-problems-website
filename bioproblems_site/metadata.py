...
class LibreTextsLink:
	url: str
	# unit is 0 when the subject's LibreTexts book has no "Unit N" label
	# (e.g., Advanced Genetics is chapter-only).
	unit: int
	chapter: int
...
def _validate_libretexts(payload: dict, topic_key: str) -> "LibreTextsLink | None":
	...
	# unit is optional (0 means chapter-only books like Advanced Genetics).
...
def _build_topic(topic_key: str, payload: dict) -> Topic:
	...
	allowed = {"title", "description", "libretexts", "visible"}
	unknown = set(payload.keys()) - allowed
	if unknown:
