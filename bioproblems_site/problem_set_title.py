...
# local repo modules
import bioproblems_site.llm_wrapper
...
def run_ollama(prompt: str, model: str | None = None) -> str:
	...
	"""
	if not model:
		model = bioproblems_site.llm_wrapper.select_ollama_model()
	return bioproblems_site.llm_wrapper.query_ollama_model(prompt, model, quiet=True)
...
def get_problem_title_from_response(response_content: str) -> str:
	...
	title = bioproblems_site.llm_wrapper.extract_xml_tag(response_content, "title")
